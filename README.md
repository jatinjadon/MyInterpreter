...
Designed with a strict adherence to **modern C++ paradigms**, this project utilizes `std::variant` for runtime dynamic typing, smart pointers (`std::unique_ptr` and `std::shared_ptr`) for safe AST memory management, and robust template metaprogramming for implementing the Visitor pattern. It serves as an excellent foundation for exploring compiler design, lexical analysis, recursive descent parsing, and semantic resolution.
...
* **Strict Visitor Pattern:** AST traversal (used heavily by both `Resolver` and `Interpreter`) decouples the tree structure from the specific execution or resolution logic being performed on it.
